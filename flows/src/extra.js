...
function FlowStore() {
  ...
  //   console.log(error);
  // };
  const executeFlow = async (value, state, flowID) => {
    console.log(value);
    try {
      const flowdata =
        state === "continue"
  ...
  const onFlowInit = async (init, data) => {
    try {
      // setHasError(false);
      // setIsLoading(true);
      // Ternary check if state = init => init state : continue state
      const response = init
        ? await executeFlow(data.instanceID, "init")
        : await executeFlow(data.values, "continue", data.id);
      setIsLoading(false);
  ...
  const onFlowBackAction = async (ID) => {
    try {
      setHasError(false);
      setIsLoading(true);
      const response = await executeFlow(ID, "back");
      setIsLoading(false);
      setFlow(response);
    } catch (error) {
      console.log(error);
